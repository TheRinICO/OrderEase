	...
	<view style="padding: 20rpx;">
		<view>
			<view>
					...
					<view style="display: flex; grid-gap: 30rpx; margin-bottom: 10rpx;"  @click="goToDeatil(item.business.id)">
						...
						<view style="flex: 1; display: flex; flex-direction: column; justify-content: space-between; grid-gap: 10rpx;">
							...
							<view style="display: flex;  color: #666; ">
								<view style="flex: 1;">
									<text style="color: red; font-weight: bold;">{{item.business.score}}分</text>
              ...
              <view style="color: #5e5e5e; border-radius: 4rpx; width: fit-content;">{{ item.business.info || '商家介绍' }}</view>
						</view>
					</view>
...
<script>
	export default {
		...
		methods: {
			...
			load() {
				// 获取商家的列表
				this.$request.get('/collect/getUserCollect/'+this.user.id).then(res => {
