<template>
	<view>
		 <uni-segmented-control :current="current" :values="items" 
			@clickItem="onClickItem" styleType="text" activeColor="#409EFF"></uni-segmented-control>
		 <view style="padding: 20rpx;">
			<view>
				<view class="box" v-for="item in ordersList" :key="item.id" style="margin-bottom: 10rpx;">
					<view style="display: flex; align-items: baseline; margin-bottom: 10rpx;justify-content: space-between;">
						<view>{{ item.businessName }}  
							<uni-icons type="right" size="16" color="#666" style="position: relative; top: 2rpx;"></uni-icons></view>
						<view style="font-size: 24rpx; color: #666;">{{ item.status }}</view>
					</view>
					<view @click="goOrdersItem(item.id)" style="display: flex; align-items: center; grid-gap: 20rpx; margin-bottom: 10rpx;">
						...
						<view style="flex: 1;">
						 ...
						 <view style="flex: 1; text-align: right;">
							 ...
							 <uni-tag v-if="item.status === '待发货'" text="申请退款" size="mini" type="error" style="margin-right: 10rpx;" @click="changeStatus(item, '已退款')"></uni-tag>
							  ...
							  <uni-tag @click="goComment(item.id)" v-if="item.status === '待评价'" text="评价" size="mini" type="royal"></uni-tag>
						 </view>
					</view>
				</view>
			</view>
