    ...
    <div class="table">
      <el-table :data="tableData" strip @selection-change="handleSelectionChange">
        ...
        <el-table-column label="详情" align="center" width="90" fixed="right">
          <template v-slot="scope">
            <el-button size="mini" type="primary"  @click="loadDetail(scope.row.id)">查看详情</el-button>
          </template>
        </el-table-column>
        <el-table-column label="操作" align="center" width="240" fixed="right">
          <template v-slot="scope">
            <el-button size="mini" type="success" @click="sendGoods(scope.row)" v-if="scope.row.status === '待发货'">发货</el-button>
            <el-button size="mini" type="primary"  @click="handleEdit(scope.row)" v-if="user.role === 'ADMIN'">编辑</el-button>
            <el-button size="mini" type="danger"  @click="del(scope.row.id)" v-if="user.role === 'ADMIN'">删除</el-button>
          </template>
        </el-table-column>
      </el-table>
